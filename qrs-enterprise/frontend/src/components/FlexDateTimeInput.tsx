...
import { useCalendar } from '../lib/calendarSetting';
...
/** Accept JSON number values as well as numeric strings from older API data. */
function parseEpoch(value: number | string): number | null {
...
export function FlexDateTimeInput({ label, kind, epoch = kind === 'datetimeEpoch', value, onChange, helperText, fullWidth, disabled }: Props) {
  ...
  const currentDate = useMemo<Date | null>(() => {
    ...
    // datetimeEpoch values are UTC seconds, not JavaScript milliseconds.
    if (kind === 'datetimeEpoch' && epoch !== null) {
  ...
  useEffect(() => {
    console.info('[QRS][DateTime] value state', {
      kind,
      calendar,
      inputType: typeof value,
      inputValue: value,
      parsedDate: currentDate?.toISOString() ?? null,
    });
  }, [calendar, currentDate, kind, value]);
  ...
  const shownInCalendar = useMemo(() => {
    ...
    try {
      ...
      console.error('[QRS][DateTime] calendar conversion failed', { kind, calendar, value, currentDate, error });
      return g;
    }
  ...
  const handleSelectDate = (d: Date): void => {
    ...
    try {
      ...
      console.error('[QRS][DateTime] date selection failed', { kind, calendar, selectedDate: d, error });
    }
  };
