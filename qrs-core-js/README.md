...
This package is the reference implementation. A Python port lives in
`src/qrs-core-python`. The conformance suite (`test/conformance.test.ts`)
proves the two cores are **wire-compatible**:

- **Golden vectors** — a shared `src/qrs-conformance/fixtures/golden.json`
  (generated by `src/qrs-conformance/generate-fixtures.mjs`) is consumed by both
  suites; each asserts it produces byte-identical canonical CBOR, hashes, key
  ids, identifiers and transfer envelopes.
- **Bidirectional cross-verification** — objects signed by the JS core verify in
  the Python core, and objects signed by the Python core verify in the JS core,
  for both Ed25519 and ECDSA-P256.
...
The conformance tests require the Python core's venv
(`src/qrs-core-python/.venv`) for the Python verifier bridge.
