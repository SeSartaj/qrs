...
import { createSecurePrivateKeyStore, type SecurePrivateKeyStore } from './securePrivateKeyStore.js';
...
export function createDesktopRuntime(getWindow: ContextWindowProvider): DesktopRuntime {
  ...
  const privateKeyStore = createSecurePrivateKeyStore(stores.privateKeyStore, config.get().privateKeyPasswordConfigured === true);
  const qrs = createQrs({
    ...stores,
    privateKeyStore,
    contextProvider: context,
  });
  return { qrs, privateKeyStore, context, dataDir, secureKeys, config };
}
